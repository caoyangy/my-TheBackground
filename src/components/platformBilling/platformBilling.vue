<template>
  <div class="flex side_sdx ivuInput-box">
	...
	<div class="H_t flex side_sdx mainAlign_center">
		<div>自营订单管理</div>
		<div>
			<router-link class="colorCCC" to="/agentBill">首页</router-link>&nbsp;&nbsp; /&nbsp;&nbsp;
			<router-link class="colorCCC blod" to="/agentBill">平台交易账单</router-link>
		</div>
	</div>
    ...
    <div class="list">
		...
		<div class="search_box">
			<Form :model="queryData" ref="queryData" label-position="right" inline>
                ...
                <FormItem label="商户名" prop="searchName" :label-width="50">
					<Input :style="input_width" v-model="queryData.searchName" placeholder="请输入商户名"></Input>
				</FormItem>
                <FormItem label="代理商" prop="searchName" :label-width="50">
					<Input :style="input_width" v-model="queryData.searchName" placeholder="请输入代理商名称"></Input>
				</FormItem>
                <FormItem label="订单号" prop="searchName" :label-width="50">
					<Input :style="input_width" v-model="queryData.searchName" placeholder="请输入订单号"></Input>
				</FormItem>
        ...
        <div class="transaction">
            <span>总交易单数 <i>50</i></span>
            <span>分润总金额 <i>100</i></span>
            <span>手续费总金额 <i>10000</i></span>
        </div>
			<!-- 信息列表 -->
		<div class="list_data_h flex main_zdy mainAlign_around">
				...
				<div v-else class="list_data_c flex main_zdy mainAlign_around" v-for='(item,index) in dataList' :key='index'> 
					...
					<div class="lineH">{{item.orderId}}</div>
