<template>
  <div class="flex side_sdx">
    ...
    <div class="H_t flex side_sdx mainAlign_center">
      ...
      <div>
        首页&nbsp;&nbsp; /&nbsp;&nbsp;
        <router-link class="colorCCC blod" to="/allShopMsg">门店信息</router-link>
      </div>
    </div>
      ...
      <div class="list_data">
        <Form :model="queryData" ref="queryData" label-position="right" inline>
          <FormItem class="clearfix search_btn">
            <button @click="clearVal()"> 搜索 </button>
					  <button  @click="getList()"> 清空 </button>
          ...
          <FormItem label="经营品类:" prop="status" :label-width="70">
            <!-- <Select placeholder="请选择" v-model="queryData.categoryId" :style="selectStyle">
              <Option :value="item.name" v-for="(item,i) in statusList" :key="i" @click.native="delet(item.id)">{{item.label}}</Option>
			    ...
			    <el-select v-model="queryData.categoryId" size="small"  placeholder="请选择">
				    <el-option
        ...
        <div class="oppen_add_couser_model">
          <button @click=" oppenAddCouserModel ">添加课程</button>
        </div>
        <!--列表头部-->
...
export default {
  ...
  methods: {
    ...
    // 打开添加课程model
    oppenAddCouserModel: function() {
      this.addCouserModel = true;
	...
	//获取搜索列表
	getSearchList(url,current,pageSize,userid,token,names) {
		this.$API('post',url,{
			"userId": this.userid,
			// "status": '1012',
			"currentPage":current,
			...
			this.addCouserList = [];
			this.addCouserList = res.data.data.list;
		})
	},
    //获取列表数据
    getList() {
