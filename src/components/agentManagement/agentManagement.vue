	...
	<div class="flex side_sdx">
		...
		<div class="H_t flex side_sdx mainAlign_center">
			...
			<div>
				...
				&nbsp;&nbsp; /&nbsp;&nbsp;
				<router-link class="colorCCC blod" to="/agentManagement">代理商管理</router-link>
			</div>
		</div>
		<div class="list">
			...
			<div class="list_data">
				...
				<div class="list_data_c flex main_zdy mainAlign_around" v-for="(item,index) in listOrder" :key="index">
					<div :title="item.addTime">{{item.addTime}}</div>
					<div :title="item.agencyName">{{item.agencyName}}</div>
					<div :title="item.companyAddress">{{item.companyAddress}}</div>
					<div>
                        <span v-if="item.companyType=='0'">合伙人</span>
                        <span v-if="item.companyType=='1'">代理人</span>
                    </div>
					<div :title="item.principalName">{{item.principalName}}</div>
					...
					<div class="flex sideAlign_center mainAlign_center">
                        <button @click="audit(item.id,item.agencyInfoId)">查看</button>
	...
	export default {
		...
		methods: {
			...
			List(e) {
				...
				//获取列表
				this.$APIA('post', 'agencyinfomanager/getAllAgency?'+data, {
					// "agencyName": this.searchVal.searchName,
					// "pageNum": this.Page.current,
					// "pageSize":this.Page.pageSize
...
<style scoped="scoped">
	...
	.list_data_c:nth-child(1) {
			width: 160%
	}

	.list_data_c:nth-child() {
		background: #fff;
		box-sizing: border-box;
	}
	.list_data_c div {
