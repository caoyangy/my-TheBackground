  ...
  <div class="flex side_sdx ivuInput-box">
    ...
    <div class="H_t flex side_sdx mainAlign_center">
      ...
      <div>
        ...
        <router-link class="colorCCC blod" to="/commodityManagementAdd">新增商品</router-link>
      </div>
    </div>
    <!-- 表单区域 -->
    <div class="list">
      ...
      <Form :model="goodsAddForm" ref="goodsAddForm" :rules="goodsAddFormRules" :label-width="90">
        ...
        <!---------------------- 商品名称 ok--------------------->
        <Card  class="listFrom" :style="font_s14" >
                ...
                >
                  ...
                  <Col span="5">
                    <FormItem prop="specGoodsThumbnailImg" :label-width="0">
                      ...
                      >
                        <img :src="goodsAddForm.attributeList[i].specGoodsThumbnailImg" alt>
        ...
        <Card style="width: 100%" class="listFrom">
          ...
          <div class="noborder-form">
            <FormItem label prop="bannerImgs">
              <Input v-model="goodsAddForm.bannerImgs"></Input>
            </FormItem>
          </div>
        </Card>
        <!---------------------- 商品参数 ok--------------------->
        ...
        <Card style="width: 100%" class="listFrom">
          <div class="title">
            ...
            <!-- <RadioGroup v-model="goodsAddForm.status" vertical class="idAdded" type="button">
              <Radio :label="item.label" v-for="item in statusList" :key="item.label" style="margin-left:20px" type="success">
                <span>{{item.text}}</span>
              </Radio>
            </RadioGroup> -->
          </div>
        ...
        <Card style="width: 100%" class="listFrom">
          <div class="title">
            ...
            <!-- <RadioGroup v-model="goodsAddForm.recommend" vertical class="idAdded" type="button">
  ...
  methods: {
    ...
    imgHandel() {
      ...
      arry.forEach(function(item, i) {
        newArry.push(item.response.data.url);
      });
      //console.log(newArry)
        ...
        if (valid) {
          ...
          for (var i in formdatas) {
            data +=
              ...
              "=" +
              encodeURIComponent(formdatas[i]) +
              "&";
          }
          data = data.slice(0, -1);
          this.$APIMy(
    ...
    specGoodsImgSuccess(res, file) {
      ...
      let i = this.i;
      this.goodsAddForm.attributeList[i].specGoodsThumbnailImg = res.data.url;
    },
    // 商品缩略图上传成功
    goodsImgSuccess(res) {
      //console.log(res.data);
