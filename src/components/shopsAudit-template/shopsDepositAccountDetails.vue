	...
	<div class="flex side_sdx wary">
		...
		<div class="H_t flex side_sdx mainAlign_center">
			...
			<div>
                ...
                &nbsp;&nbsp; /&nbsp;&nbsp; 
                <router-link class="colorCCC " to="/shopsDepositAccount">商家存管账户</router-link> 
				&nbsp;&nbsp; /&nbsp;&nbsp; 
                <router-link class="colorCCC blod" to="/shopsDepositAccountDetails">详情</router-link>
            </div>
		</div>
		<!--tab切换-->
		<div class="tab flex main_zdy sideAlign_center">
			<a v-for="(item,index) in tab" :key="index" :class="tabname == item?'tab_don' :'tab_d'" @click="tabon(item,'#shops'+index)">
				<div>{{item}}</div>
			</a>
		</div>
		...
		<div class="OrderDetails flex main_zdy" id="shops0">
			<div class="OrderDetails_l flex side_sdx">
				...
				<div class="flex side_sdx mainAlign_between" style="height: 100%;padding-top: -45px;">
					<div class="message border_top_none">
			...
			<div class="OrderDetails_r">
				...
				<div class="picture flex main_zdy">
					...
					<!-- <div class="pictureimg" @click="imgon(details.idcardHandUrl)">
						<img :src="details.idcardHandUrl" :onerror="onerror"/>
						<div>手持身份证</div>
					</div> -->
		...
		<div class="three cunguan_three" id="shops1">
			<div class="headline  flex main_zdy sideAlign_center cunguan_zhanghu">
				存管账户
		...
		<!--初审-->
		<div class="hide flex sideAlign_center mainAlign_center" v-if="firstshow">
			<div class="hide_d">
		...
		<div class="hide flex sideAlign_center mainAlign_center" v-if="againshow">
			<div class="hide_d">
				...
				<div class="limit_parent">
					<input type="text" placeholder="建议额度" class="limit" v-if='again == "1010"' ref='againlimit' /> <span>元</span>
				</div>
				<div  class="limit_parent">
					<input type="text" placeholder="商家扣占比" class="limit margin_b_none " v-if='again == "1010"' @blur="PercentBlur" ref='againPercent' /><span>%</span>
				</div>
				<div class="clear"></div>
				<div class="percent_msg">每笔课程款中商家支出的服务费和补贴占总金额的比例</div>
				...
				if(cause != '' || this.selectCaiFu != 1) {
					//状态提交
						API('post','examineLog/create',{
							...
							type: this.again,
							createBy: this.userid,
							p2pPlatformId : this.selectCaiFu*1,
							rate : 	this.PercentNum/100,
							creditLine : limit,
						},this.userid,this.token,(res)=>{
							this.content = "复审成功";
							this.infos();
